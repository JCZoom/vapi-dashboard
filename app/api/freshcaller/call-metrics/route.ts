...
export async function GET(request: NextRequest) {
  try {
    ...
    let url = `${baseUrl}/call_metrics`;
    if (callId) {
      url = `${baseUrl}/calls/${callId}/call_metrics`;
    }
    ...
    console.error('Freshcaller call metrics error:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to fetch call metrics' },
      { status: 500 }
    );
  }
}
